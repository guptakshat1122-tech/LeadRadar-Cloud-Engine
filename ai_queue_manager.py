...
CLAIM_TTL_SECONDS = int(
    os.environ.get("AI_CLAIM_TTL_SECONDS", 12 * 3600)
)
...
# ============================================================
# GIT IDENTITY INITIALIZATION
# GitHub Actions runners may not have identity configured.
# ============================================================

subprocess.run(
    ["git", "config", "user.name", "Bawa Cloud Engine"],
    cwd=BASE_DIR,
    check=True,
)
...
def claim_path(domain):
    digest = hashlib.sha256(
        normalize_domain(domain).encode("utf-8")
    ).hexdigest()
...
def load_completed_domains():
    ...
    # Result shards are also treated as authoritative
    # completion evidence before canonical merge.
...
def load_active_claimed_domains():
    ...
    for filename in files:
        ...
        try:
            ...
            # Backward-compatible:
            # old claim files may contain:
            #   "domain": "example.com"
            #
            # newer format may contain:
            #   "domains": [...]
            ...
            if isinstance(domains, list):
                ...
                single_domain = payload.get(
                    "domain",
                    "",
                )

                raw_domains = [single_domain]
...
def all_xray_rows():
    ...
    for date_str in sorted(
        os.listdir(PROCESSING_QUEUE_DIR)
    ):
...
def claim_batch(worker, batch_size):
    ...
    for attempt in range(4):
        ...
        # Safety: after git pull/rebase, make absolutely sure
        # the claim directory exists.
        os.makedirs(
            CLAIMS_DIR,
            exist_ok=True,
        )
        ...
        # git pull/rebase can recreate the working tree state.
        # Guarantee claim directory again after synchronization.
        os.makedirs(
            CLAIMS_DIR,
            exist_ok=True,
        )
        ...
        try:
            ...
            for row in candidates:
                ...
                # Guarantee parent directory directly
                # before creating the claim file.
                os.makedirs(
                    CLAIMS_DIR,
                    exist_ok=True,
                )
            ...
            try:
                ...
                # one of the same domains first.
                #
                # Roll our local claim commit back
                # to the exact pre-claim SHA.
                git(
                    "reset",
                    "--hard",
                    base_sha,
            ...
            # Remove only files from a claim
            # that never became part of remote.
            for path in created_files:
...
def publish_result_and_release(
    ...
    # --------------------------------------------------------
    # Release claim locks in second transaction.
    # --------------------------------------------------------
    ...
    remaining_rel = [
        ...
        for p in claim["claim_files"]
        if not os.path.exists(p)
    ]

    # Even when no result was produced,
    # deleting the claims is a real change.
    #
    # If a result was published, that result commit
    # is already remote; now release locks.
...
def merge_result_shards():
    ...
    if os.path.isdir(RESULTS_DIR):
        ...
        for filename in sorted(
            ...
            # Result shard is consumed once
            # incorporated into canonical registry.
            try:
                os.remove(path)
...
def archive_ready_dates(master_dir):
    ...
    for date_str in sorted(
        ...
        if source_domains and not source_domains.issubset(
            set(classified)
        ):
            continue
...
def publish_result_checkpoint(
    ...
    """
    Publish a partial/complete result shard
    without releasing claims.
    """
